...
// M�todo Bar�basi
    ...
    public grafo Dijkstra(vertice fuente) {
        ...
        for (int k = 0; k < this.Aristas.size(); k++) {
            COSTOS[this.Aristas.get(k).getan_from()][this.Aristas.get(k).getan_to()] = this.Aristas.get(k).getP();
            COSTOS[this.Aristas.get(k).getan_to()][this.Aristas.get(k).getan_from()] = this.Aristas.get(k).getP();
        }
